...
CREATE TABLE schools (
    ...
    board           VARCHAR(30)  NOT NULL DEFAULT 'CBSE',
    address         TEXT,
    city            VARCHAR(100),
    state           VARCHAR(100),
    pincode         VARCHAR(10),
...
CREATE TABLE students (
    id              UUID        PRIMARY KEY DEFAULT gen_random_uuid(),
    --user_id         UUID        UNIQUE REFERENCES users(id) ON DELETE SET NULL, Temporary Commented
    school_id       UUID        NOT NULL REFERENCES schools(id) ON DELETE CASCADE,
    ...
    --house_id        UUID        REFERENCES houses(id),  --Drooped
    address         TEXT, --student_addresses  Table can be ignore
    is_active       BOOLEAN       NOT NULL DEFAULT TRUE,
...
INSERT INTO subscription_plans (
    ...
    199.00,
    1999.00,
    '["attendance", "fees", "basic reports"]'::jsonb,
    true,
    1
);